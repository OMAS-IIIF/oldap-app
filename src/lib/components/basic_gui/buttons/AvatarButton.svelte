...
<div class="relative inline-block text-left">
	<button class={class_str}
		...
		{#if src}
			<img {id} class="{userClass} inline-block size-10 rounded-full" src={src} alt="" onerror={no_avatar} />
			{#if avatar_error}
				<span {id} class="{userClass} text-medium font-medium text-white bg-black rounded-full">{initials}</span>
			{/if}
		{:else if initials}
			 <span {id} class="{userClass} text-medium font-medium text-white bg-black rounded-full">{initials}</span>
		{:else}
			<svg class="{userClass} size-full text-gray-300" fill="currentColor" viewBox="0 0 24 24">
