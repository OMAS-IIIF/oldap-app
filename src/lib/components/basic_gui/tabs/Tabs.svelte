	...
	<div class="hidden sm:block">
		<div class="border-b border-gray-200">
			<nav class="-mb-px flex" aria-label="Tabs">
				...
				{#each Object.entries(tabs) as [id, name]}
					{#if selected === id}
						<a href="#" class="w-1/4 border-b-2 border-indigo-500 px-1 py-4 text-center text-sm font-medium text-indigo-600" aria-current="page">{name}</a>
					{:else}
						<a href="#" class="w-1/4 border-b-2 border-transparent px-1 py-4 text-center text-sm font-medium text-gray-500 hover:border-gray-300 hover:text-gray-700">{name}</a>
					{/if}
				{/each}
			</nav>
		</div>
