<script lang="ts">
	...
	onMount(async () => {
		...
		if (data.roleid !== 'new') {
			...
			const config_role = api_notget_config(authinfo as AuthInfo, {
				definedByProject: definedByProject || '',
				roleId: data.roleid
			});
			apiClient.getAdminroleDefinedByProjectRoleId(config_role).then((data) => {
	...
	const add_role = async () => {
		confirmation_title = m.add_role();
		confirmation_message = m.confirm_role_add({roleid: roleId.toString()});
		...
		apiClient.putAdminroleDefinedByProjectRoleId(roledata, role_put).then((res) => {
			successInfoStore.set(`Rle "${roleId}" added successfully!`);
		}).catch((error) => {
			errorInfoStore.set(process_api_error(error as Error));
		});
	};

	const modify_role = async () => {
		confirmation_title = m.modify_role();
		confirmation_message = m.confirm_role_modify({roleid: role?.roleId.toString() || ''});
		const ok = await confirmation_dialog.open();
		if (!ok) return;
		...
		if (roledata) {
			...
			apiClient.postAdminroleDefinedByProjectRoleId(roledata, role_post).then((res) => {
				successInfoStore.set(`Permission set "${res.roleid}" modified successfully!`);
			}).catch((error) => {
				errorInfoStore.set(process_api_error(error as Error));
			});
		}
...
<div class="absolute top-0 left-0 right-0 bottom-0 overflow-auto flex flex-col justify-center items-center" bind:this={topwin}>
	<div>{data.roleid !== 'new' ? m.edit()  : m.add()} {m.role()} </div>
	<form class="max-w-128 min-w-96">
		...
		<div class="flex justify-center gap-4 mt-6">
			...
			{#if data?.roleid === 'new'}
				<Button class="mx-4 my-2" onclick={() => add_role()}>{m.add()}</Button>
			{:else}
				<Button class="mx-4 my-2" onclick={() => modify_role()}>{m.modify()}</Button>
			{/if}
		</div>
